...
def parse_status_and_preview(raw: bytes, max_preview: int = 200) -> tuple[Optional[int], str, Optional[str]]:
    ...
    try:
        ...
        return None, "", "Failed to decode HTTP header"
    
    lines = header.split("\r\n")

    if not lines or not lines[0]:
        return None, "", "Missing HTTP status line"
    ...
    try:
        ...
        return None, "", "Invalid HTTP status code"

    try:
        preview = body[:max_preview].decode("utf-8")
    except UnicodeDecodeError:
        return None, "", "Failed to decode HTTP body"
